...
app.get("/api/farmer/me", (req, res) => {
    const google_id = req.query.google_id;  

    if (!google_id) return res.status(400).json({ error: "Missing Google ID" });

    const sql = "SELECT id, google_id, full_name, profile_pic FROM Farmers WHERE google_id = ?";
    db.get(sql, [google_id], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.get("/api/project/:project_id", (req, res) => {
